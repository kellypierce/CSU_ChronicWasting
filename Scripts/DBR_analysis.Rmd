...
for(file in qual.counts){
  ...
  print(min(counts.fraction))
  print(min(counts.fraction)*sum(dbr.counts[,1]))
  ...
  hist(counts.logit-min(counts.logit), main=paste('Minimum Median Quality = ', substr(file, 1, 2)), xlab='logit(Fraction of Reads with DBR)')
}
```